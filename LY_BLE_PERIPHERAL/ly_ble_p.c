...
static void advertising_init(void)
{
    ...
    ble_uuid_t adv_uuids[] = {{BLE_UUID_TUS_SERVICE, m_tus.uuid_type}};
    ...
    err_code = sd_ble_gap_adv_set_configure(&m_adv_handle, &m_adv_data, &adv_params);//What's the meaning of m_dav_handle?
    APP_ERROR_CHECK(err_code);
}
#endif
...
static void services_init(void)
{
	...
	//tus_init initialize
	tus_init.data_handler = ble_tus_data_handler;

    err_code = ble_tus_init(&m_tus, &tus_init);
    APP_ERROR_CHECK(err_code);
...
static void on_disconnect(ble_gap_evt_t const * const p_gap_evt)
{
	uint32_t periph_link_cnt = ble_conn_state_peripheral_conn_count();
...
Issue 001:
	Issue Description:
		The iphone already bonding with device, when the device reboot, the iphone does not reconnect to the device?
	Issue Solution:
